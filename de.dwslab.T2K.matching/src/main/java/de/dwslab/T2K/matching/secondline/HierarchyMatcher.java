        ...
        try {
            new Parallel<TFirst>(isRunInParallel() ? 0 : 1).foreach(getFirstMatrix().getFirstDimension(), new Consumer<TFirst>() {
                ...
                public void execute(TFirst parameter) {
                    final Collection<TSecond> firstParts = getHierarchy().getParts( parameter);
                    ...
                    for (final TFirst candidate : getFirstMatrix().getMatches(parameter)) {
                        ...
                        final Double firstScore = getFirstMatrix().get(parameter,candidate);

                        final Collection<TSecond> secondParts = getHierarchy().getParts(candidate);

                        calculateScoreForFirstMatrix(parameter, candidate,firstParts, secondParts, firstScore);
