...
public class PropertyBasedClassRefinementComponent extends WebtableToDBpediaMatchingComponent {
    ...
    private int numberOfPropMappings;
    private int numberOfInstanceMappings;
    //default
...
//    public static final Parameter PAR_PROP_FINAL_THRESHOLD = new Parameter("Iterative.Property.Final_threshold", 0.05);
    ...
    protected void initialiseParameters(Configuration config) {
        ...
        //IterativeComponent it = getMatchers().get
    ...
    public void run(Configuration config) {
        ...
        LabelBasedSchemaMatcher lbs = new LabelBasedSchemaMatcher(getSimilarities(), getMatchingParameters(), getRootTimer(), getGoldStandard(), getLogger());
        ...
        //why are all properties taken into account and not the ones of the selected classes?
        ...
        getMatchers().getPropertyMatcher().setClassSimilarity(getMatchers().getCandidateRefinementMatcher().getClassSimilarity());
        ...
        getMatchers().getPropertyMatcher().setLabelSimilarity(allLabelSimilarity);
        ...
        // DO IT OR NOT?
        if (propertySimilarity == null || logResult.getPropertyMappings() == null) {
            numberOfPropMappings = 0;
        } else {
            //mapProperties(propertySimilarity, getWebTableName(), false, logResult);
            numberOfPropMappings = logResult.getPropertyMappings().size();
        ...
        candidateSimilarity = getSimilarities().getCandidateSimilarity();
        //TEST!!!
        ...
        for (TableColumn c : finalClass.getColumns()) {
            if (c.getURI().contains("/property/")) {
                continue;
            }
            int currentCount = 0;
            for (int i : c.getValues().keySet()) {
                if (c.getValues().get(i) != null) {
                    currentCount++;
                }
            }
            if (counts.containsKey(c.getURI())) {
                int j = counts.get(c.getURI());
                j += currentCount;
                counts.put(c.getURI(), j);
            } else {
                counts.put(c.getURI(), currentCount);
            }
        }
//        for (String s : counts.keySet()) {
//            if (counts.get(s) < (int) finalClass.getKey().getValues().size() * 1.0) {
//                System.out.println(s + " --- " + counts.get(s) + " --- " + (int) finalClass.getKey().getValues().size() * 1.0);
//            }
//        }
...
//                if (propertySimilarity.get(c, d) ==null && getSimilarities().getLabelSimilarity().get(c, d)>0.5) {
//                    combinedValues.set(c, d, getSimilarities().getLabelSimilarity().get(c, d));

//                if (counts.get(d.getURI()) < (int) finalClass.getKey().getValues().size()*0.5) {
//                    System.out.println("label Sim: " + c.getHeader() + " -- " + d.getHeader() + " -- " + getSimilarities().getLabelSimilarity().get(c, d));
//                    combinedValues.set(c, d, getSimilarities().getLabelSimilarity().get(c, d));
//                } else {

//                    if (propertySimilarity.get(c, d) == null) {
...
//                        }
            ...
            //combinedValues.makeStochastic();
        }
//                }
//            }
//        }
        //propertySimilarity = combinedValues.copy();
...
//        numberOfInstanceMappings = logResult.getInstanceMappings().size();
//
//        mapProperties(propertySimilarity, getWebTableName(), false, logResult);
//        propertyResult = logResult.getEvaluation().getPropertyResult();
    }
    ...
    /**
     * @return the numberOfPropMappings
     */
    public int getNumberOfPropMappings() {
        return numberOfPropMappings;
    }

    /**
     * @param numberOfPropMappings the numberOfPropMappings to set
     */
    public void setNumberOfPropMappings(int numberOfPropMappings) {
        this.numberOfPropMappings = numberOfPropMappings;
    }

    /**
     * @return the numberOfInstanceMappings
     */
    public int getNumberOfInstanceMappings() {
        return numberOfInstanceMappings;
    }

    /**
     * @param numberOfInstanceMappings the numberOfInstanceMappings to set
     */
    public void setNumberOfInstanceMappings(int numberOfInstanceMappings) {
        this.numberOfInstanceMappings = numberOfInstanceMappings;
    }
    /**
