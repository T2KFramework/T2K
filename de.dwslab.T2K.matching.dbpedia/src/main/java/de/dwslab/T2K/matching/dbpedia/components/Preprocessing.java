...
//import de.mannheim.uni.index.surfaceForms.SurfaceFormIndex;
//import de.mannheim.uni.index.surfaceForms.SurfaceFormIndexEntry;
...
//    private static SurfaceFormIndex i;
    ...
    public static void addRedirects(Table t, boolean addSFForKeyColumns, MatchingData data) throws FileNotFoundException, IOException {
//        if (redirects == null) {
//            Timer tLoad = Timer.getNamed("Load Redirects", null);
//            redirects = new HashMap<>();
//            BufferedReader read = new BufferedReader(new FileReader(new File(redirectPath)));
...
//        }
    ...
    public static void addSurfaceForms(Table t, boolean addSFForKeyColumns, MatchingData data) throws FileNotFoundException, IOException {
//        if (surfaceForms == null) {
...
//                            c.getValues().put(key, allForms);
//                        }
                        if (c.getValues().get(key) != null && surfaceForms.get((String) c.getValues().get(key)) != null) {
...
//                                }
    ...
    protected static String getLabelByPart(String uriPart, MatchingData data) {
        TableRow row = data.getCandidateMap().get("http://dbpedia.org/resource/" + uriPart);

        if (row != null) {
            return (String) new TableRowMatchingAdapter().getLabel(row);
        } else {
...
//    }
    ...
    public static void handleIncorrectDates(Table t) {
        for (TableColumn c : t.getColumns()) {
            if (c.getHeader().toLowerCase().contains("releasedate") && t.toString().toLowerCase().contains("video")
