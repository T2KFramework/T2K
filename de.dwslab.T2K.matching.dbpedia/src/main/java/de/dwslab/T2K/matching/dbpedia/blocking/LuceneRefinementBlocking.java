 ...
 */
package de.dwslab.T2K.matching.dbpedia.blocking;
...
public class LuceneRefinementBlocking<T> extends Blocking<T> {
    ...
    public LuceneRefinementBlocking(IIndex index, MatchingAdapter<T> labelAdapter, MatchingAdapter<T> keyAdapter, SimilarityMatrix<T> candidateSimilarities, Map<String, T> candidateMap, Collection<String> refinementClasses) {
        setIndex(index);
    ...
    private Map<String, T> candidateMap;
    
    public Map<String, T> getCandidateMap() {
        return candidateMap;
    }
    ...
    public Collection<T> getCandidates(T instance, Collection<T> candidates) {
        ...
        if (lbl != null) {
            ...
            for (DBpediaIndexEntry e : matches) {
                if (e.getUri() != null
                        && candidateMap.containsKey(e.getUri())) {
                    T cand = candidateMap.get(e.getUri());
                    
                    lookup += System.currentTimeMillis() - start;
                    start = System.currentTimeMillis();
                    
                    // add a candidate only if it is not already included in the similarity matrix
                    if(!getCandidateSimilarities().getSecondDimension().contains(cand)) {
                        result.add(cand);
                        newCandidates++;
                    }
