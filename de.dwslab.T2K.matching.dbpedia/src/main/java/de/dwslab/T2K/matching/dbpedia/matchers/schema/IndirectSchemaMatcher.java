 ...
 * @author dritze
 */
public class IndirectSchemaMatcher {
    
    private SimilarityMatrix<TableColumn> overallLabel;
    private SimilarityMatrix<TableColumn> overallValue;
    

    public SimilarityMatrix<TableColumn> computeIndirectMappings(MatchingParameters matchingParameters, Timer rootTimer,
        ...
        //unmappedCols = data.getWebtable().getColumns();
...
//                                    + c2.getTable());
        ...
        overallLabel = new SparseSimilarityMatrix<>(unmappedCols.size(), data.getDbpediaColSet().size());
        overallValue = new SparseSimilarityMatrix<>(unmappedCols.size(), data.getDbpediaColSet().size());
        ...
        for (TableColumn r1 : getOverallLabel().getFirstDimension()) {
            for (TableColumn r2 : getOverallLabel().getMatches(r1)) {
                if (labelColumnCount.get(r1) != null) {
                    int occurence = labelColumnCount.get(r1);
                    System.out.println("label " + r1 + " - " + r2 + " - " +getOverallLabel().get(r1, r2) + " - " +occurence + " - " +getOverallLabel().get(r1, r2) / occurence);
                    getOverallLabel().set(r1, r2, getOverallLabel().get(r1, r2) / occurence);
                }
            }
        }

        for (TableColumn r1 : getOverallValue().getFirstDimension()) {
            for (TableColumn r2 : getOverallValue().getMatches(r1)) {
                if (valueColumnCount.get(r1) != null) {
                    int occurence = valueColumnCount.get(r1);
                    System.out.println("value " + r1 + " - " + r2 + " - " +getOverallValue().get(r1, r2) + " - " +occurence+ " - " +getOverallValue().get(r1, r2) / occurence);
        ...
        for (TableColumn r : getOverallLabel().getFirstDimension()) {
            List<TableColumn> l = new ArrayList<>();
            l.addAll(getOverallLabel().getMatches(r));
            possibleMatchesByRow.put(r, l);
        }
        for (TableColumn r : getOverallValue().getFirstDimension()) {
            List<TableColumn> l = new ArrayList<>();
...
//        combine.setSecondWeight(stats.get(overallLabel).getMean());
    ...
    //try with real mean...
        ...
        y2.multiplyScalar(stats.get(getOverallValue()).getMean());
        SimilarityMatrix x2 = getOverallLabel().copy();
        x2.multiplyScalar(stats.get(getOverallLabel()).getMean());
        
        combine.setAggregationType(CombinationType.Sum);
        SimilarityMatrix<TableColumn> overall1 = combine.match(y2,x2);
...
//        averageCounts = averageCounts / countR2.size();
        ...
        for (TableColumn r1 : overall.getFirstDimension()) {
            ...
            if(getOverallValue().get(r1, best)==null || best.getURI().equals("http://dbpedia.org/ontology/internationally")) {
               continue;
            }
            if(overall.get(r1, best)<1.0) {
                 System.out.println("too low: " +overall.get(r1, best) + "\t" + best.getURI() + "\t" + overall.get(r1, best));
                continue;
            }
            double rel = (double)valueColumnCount.get(r1)/(double)dbpediaPropsPerTable.size();
            if(overall.get(r1, best)==1.0 && rel<1) {
                System.out.println("relation too low: " +overall.get(r1, best) + " - " + rel + " - " + valueColumnCount.get(r1) + " - "+ (double)dbpediaPropsPerTable.size() + "\t" + best.getURI()  + "\t" + overall.get(r1, best));
                continue;
            }
            ...
            for (Object o : gs.getPropertyGoldStandard().keySet()) {
                ...
                if (i == r1.getTable().getColumns().indexOf(r1)) {
                    ...
                    if (evalInstance.getUniqueIdentifier(best).equals(gs.getPropertyGoldStandard().get(o))) {
                        System.out.println("final overall DBpedia prop: " + r1.getTable().getHeader() + "\t" + r1 + "\t" + evalInstance.getUniqueIdentifier(best) + "\t" + overall.get(r1, best) + "\t" + getOverallValue().get(r1, best) + "\t"
                                + getOverallLabel().get(r1, best) + "\t" + labelColumnCount.get(r1) + "\t" + valueColumnCount.get(r1)
...
//                        }
        ...
        for (TableColumn tc1 : possibleMappingsCosine.keySet()) {
            if (possibleMappingsCosine.get(tc1) == null || possibleMappingsCosine.get(tc1).isEmpty()) {
                continue;
            }
     ...
     */
    public SimilarityMatrix<TableColumn> getOverallLabel() {
        return overallLabel;
    }
     ...
     */
    public SimilarityMatrix<TableColumn> getOverallValue() {
        return overallValue;
    }
