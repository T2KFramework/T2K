...
import static de.dwslab.T2K.matching.dbpedia.mainInstance.getTime;
...
//            }
            ...
            final PropertyMatchingTask m = new PropertyMatchingTask();
                        ...
                        new Parallel<File>().foreach(files, new Consumer<File>() {
                            ...
                            public void execute(File parameter) {
                                //if(!matchedTables.contains(parameter.getName())) {
                                    try {
                                        PropertyMatchingTask p = new PropertyMatchingTask();
                                        ...
                                        if(r!=null && (Boolean)c.getValue(PropertyMatchingTask.PAR_EVALUATE)) {
                                            if (r.getEvaluation().getPropertyResult() != null) {
                                                synchronized (results) {
                                                    resultF1.AddValue(r.getEvaluation().getPropertyResult().getF1Score());
                                                    results.add(r);
                                                }
                                            }
                                        }
