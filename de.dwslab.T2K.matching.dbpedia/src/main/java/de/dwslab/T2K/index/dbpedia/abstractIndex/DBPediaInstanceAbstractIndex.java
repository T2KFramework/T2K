...
public class DBPediaInstanceAbstractIndex extends IndexManagerBase {
    ...
    public DBPediaInstanceAbstractIndex(IIndex index, String defaultField) {
    ...
    public List<DBpediaAbstractIndexEntry> searchMany(Collection labels) {
        long start, setup = 0, search = 0, result = 0;
        start = System.currentTimeMillis();
        List<DBpediaAbstractIndexEntry> results = new LinkedList<DBpediaAbstractIndexEntry>();
            ...
            if (q != null) {
                ...
                if (hits != null) {
                    ...
                    for (int i = 0; i < hits.length; i++) {
                        ...
                        DBpediaAbstractIndexEntry e = DBpediaAbstractIndexEntry.fromDocument(doc);

                        if (isVerbose()) {
                            System.out.println(e.getUri() + ": " + e.getAbstractText());
                        }
    ...
    public List<DBpediaAbstractIndexEntry> search(String label) {
        long start, setup = 0, search = 0, result = 0;
        start = System.currentTimeMillis();
        List<DBpediaAbstractIndexEntry> results = new LinkedList<DBpediaAbstractIndexEntry>();
